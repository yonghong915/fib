package com.fib.msgconverter.commgateway.channel.config.route;
	...
	public static int getTypeByText(String typeText) {
		if (TYP_STATIC_TXT.equalsIgnoreCase(typeText)) {
			...
			throw new RuntimeException(
					MultiLanguageResourceBundle.getInstance().getString("type.unsupport", new String[] { typeText }));
		}
	}
	...
	public Determination determine(String messageType, byte[] message, Object messageObject) {
		if (type != TYP_DYNAMIC) {
			...
			throw new RuntimeException(MultiLanguageResourceBundle.getInstance()
					.getString("RouteRule.determine.type.notDynamic", new String[] { id, TYP_DYNAMIC_TXT }));
		}
