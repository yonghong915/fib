	...
	public String recognize(byte[] message) {
		if (1 == fieldNum) {
			if (index + length > message.length) {
				throw new RuntimeException("message to short! key field index=" + index + ", length=" + length);
			}
			...
			for (int i = 1; i < fieldNum + 1; i++) {
				...
				if (field.index + field.length > message.length) {
					throw new RuntimeException(
							"message to short! key field index=" + field.index + ", length=" + field.length);
				}
	...
	private void parseParameters() {
		...
		if (1 == fieldNum) {
			...
			if (null == tmp) {
				...
				tmp = tmp.trim();
				if (0 == tmp.length()) {
					throw new RuntimeException("parameter " + FLD_INX + " is blank!");
				}
			}
			...
			if (null != tmp) {
				tmp = tmp.trim();
				if (0 == tmp.length()) {
					throw new RuntimeException("parameter " + FLD_DTA_TYP + " is blank!");
				}
				this.dataType = Constant.getDataTypeByText(tmp);
				if (!(this.dataType == Constant.DTA_TYP_STR || this.dataType == Constant.DTA_TYP_NUM)) {
					throw new RuntimeException("parameter " + FLD_DTA_TYP + " is wrong! Unsupport Data Type[" + tmp
			...
			if (null == tmp) {
				...
				tmp = tmp.trim();
				if (0 == tmp.length()) {
					throw new RuntimeException("parameter " + FLD_LEN + " is blank!");
				}
			}
			...
			for (int i = 1; i < fieldNum + 1; i++) {
				...
				if (null == tmp) {
					throw new RuntimeException("parameter " + FLD + i + "." + FLD_INX + " is null!");
				} else {
					tmp = tmp.trim();
					if (0 == tmp.length()) {
						throw new RuntimeException("parameter " + FLD + i + "." + FLD_INX + " is blank!");
					}
				}
				fieldParam.index = Integer.parseInt(tmp);
				if (fieldParam.index < 0) {
					throw new RuntimeException("parameter " + FLD + i + "." + FLD_INX + " must >= 0!");
				}
				...
				if (null != tmp) {
					tmp = tmp.trim();
					if (0 == tmp.length()) {
						throw new RuntimeException("parameter " + FLD + i + "." + FLD_DTA_TYP + " is blank!");
					}
					...
					if (!(fieldParam.dataType == Constant.DTA_TYP_STR || fieldParam.dataType == Constant.DTA_TYP_NUM)) {
						throw new RuntimeException("parameter " + FLD + i + "." + FLD_DTA_TYP
								+ " is wrong! Unsupport Data Type[" + tmp + "]! data type must be num/str");
					}
				}
				...
				if (tmp != null) {
					tmp = tmp.trim();
					if (0 == tmp.length()) {
						throw new RuntimeException("parameter " + FLD + i + "." + FLD_DTA_ENC + " is blank!");
					}
					if ("bcd".equalsIgnoreCase(tmp)) {
				...
				if (null == tmp) {
					throw new RuntimeException("parameter " + FLD + i + "." + FLD_LEN + " is null!");
				} else {
					tmp = tmp.trim();
					if (0 == tmp.length()) {
						throw new RuntimeException("parameter " + FLD + i + "." + FLD_LEN + " is blank!");
					}
				}
				fieldParam.length = Integer.parseInt(tmp);
				if (fieldParam.length < 0) {
					throw new RuntimeException("parameter " + FLD + i + "." + FLD_LEN + " must > 0!");
				}
