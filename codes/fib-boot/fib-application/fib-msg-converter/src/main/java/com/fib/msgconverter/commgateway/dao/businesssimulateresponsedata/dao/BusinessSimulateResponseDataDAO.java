...
public class BusinessSimulateResponseDataDAO extends AbstractDAO {
	...
	public int insert(BusinessSimulateResponseData businessSimulateResponseData) {
		...
		try {
			...
			if(DAOConfiguration.DEBUG){
				...
				debug("BusinessSimulateResponseDataDAO.insert() spend "+(endTime - startTime)+"ms. retFlag = " + retFlag + " SQL:"+sql+"; parameters : id = \""+businessSimulateResponseData.getId() +"\",message_data = "+businessSimulateResponseData.getMessageData()+" ");
			}
	...
	public int[] insertBatch(List<BusinessSimulateResponseData> businessSimulateResponseDataList) {
		...
		try {
			...
			if(DAOConfiguration.DEBUG){
				endTime =System.currentTimeMillis();
				debug("BusinessSimulateResponseDataDAO.insertBatch() spend "+(endTime - startTime)+"ms. retFlag = " + retFlag + " SQL:"+sql+";" );
			}
			return retFlag;
		} catch (SQLException e) {
			 e.printStackTrace(System.err);
	...
	public int update(BusinessSimulateResponseData businessSimulateResponseData) {
		...
		try {
			...
			if(DAOConfiguration.DEBUG){
				endTime =System.currentTimeMillis();
				debug("BusinessSimulateResponseDataDAO.update() spend "+(endTime - startTime)+"ms. retFlag = " + retFlag + " SQL:"+sql+"; parameters : id = \""+businessSimulateResponseData.getId() +"\",message_data = "+businessSimulateResponseData.getMessageData()+" ");
			}
	...
	public int dynamicUpdate(Map primaryKey, Map updateFields) {
		...
		try {
			...
			if(DAOConfiguration.DEBUG){
				...
				sbDebug.append("BusinessSimulateResponseDataDAO.dynamicUpdate() spend "+(endTime - startTime)+"ms. retFlag = " + retFlag + " SQL:"+sql.toString()+"; parameters : ");
	...
	public int deleteByPK(String id) { 
		...
		try {
			...
			if(DAOConfiguration.DEBUG){
				endTime =System.currentTimeMillis();
				debug("BusinessSimulateResponseDataDAO.deleteByPK() spend "+(endTime - startTime)+"ms. retFlag = " + retFlag + " SQL:"+sql+"; parameters : id = \""+id+"\" ");
			}
	...
	public BusinessSimulateResponseData selectByPK (String id)  {
		...
		try {
			...
			if(DAOConfiguration.DEBUG){
				endTime =System.currentTimeMillis();
				debug("BusinessSimulateResponseDataDAO.selectByPK() spend "+(endTime - startTime)+"ms. SQL:"+sql+"; parameters : id = \""+id+"\" ");
			}
			if (resultSet.next()) {
	...
	public List findAll (int pageNum, int pageLength)  {
		...
		try {
			...
			if ("oracle".equalsIgnoreCase(DAOConfiguration.getDATABASE_TYPE())){
				sql = "select * from(select A.*,ROWNUM RN from ( select * from business_simulate_response_data ) A where rownum <= "+endNum+" ) where RN >="+startNum+" order by id";
			}else if ("mssql".equalsIgnoreCase(DAOConfiguration.getDATABASE_TYPE())){
				sql = "select * from ( select Top "+startNum+" * from (select Top "+endNum+" * from business_simulate_response_data					 ) t1)t2 order by id";
			}else if ("db2".equalsIgnoreCase(DAOConfiguration.getDATABASE_TYPE())){
				sql ="select * from ( select id,message_data, rownumber() over(order by id) as rn from business_simulate_response_data ) as a1 where a1.rn between "+startNum+" and "+endNum;
				...
				sql = "select top " + pageLength + " * from(select row_number() over (order by id) as rownumber, id,message_data from business_simulate_response_data ) A where rownumber > "  + startNum ;
			}else{
			...
			if(DAOConfiguration.DEBUG){
				endTime = System.currentTimeMillis();
				debug("BusinessSimulateResponseDataDAO.findAll()(pageNum:"+pageNum+",row count "+pageLength+") spend "+(endTime - startTime)+"ms. SQL:"+sql+"; ");
			}
			while (resultSet.next()) {
				returnDTO = new BusinessSimulateResponseData();
				returnDTO.setId(resultSet.getString("id"));
	...
	public List findByWhere (String where) {
		...
		try {
			...
			if(DAOConfiguration.DEBUG){
				endTime = System.currentTimeMillis();
				debug("BusinessSimulateResponseDataDAO.findByWhere()() spend "+(endTime - startTime)+"ms. SQL:"+sql+"; parameter : "+where);
			}
			while (resultSet.next()) {
				returnDTO = new BusinessSimulateResponseData();
				returnDTO.setId(resultSet.getString("id"));
	...
	public List findByWhere (String where, int pageNum, int pageLength)  {
		...
		try {
			...
			if ("mysql".equalsIgnoreCase(DAOConfiguration.getDATABASE_TYPE())){
				sql = "select * from business_simulate_response_data where " + where +" order by id limit "+startNum+","+pageLength;
			}else if ("oracle".equalsIgnoreCase(DAOConfiguration.getDATABASE_TYPE())){
				sql = "select * from(select A.*,ROWNUM RN from ( select * from business_simulate_response_data where  "+ where + " ) A where rownum <= "+endNum+" ) where RN >="+startNum+" order by id";
			}else if ("mssql".equalsIgnoreCase(DAOConfiguration.getDATABASE_TYPE())){
				sql = "select * from ( select Top "+startNum+" * from (select Top "+endNum+" * from business_simulate_response_data					 where " + where + " ) t1)t2 order by id";
			}else if ("db2".equalsIgnoreCase(DAOConfiguration.getDATABASE_TYPE())){
				sql = "select * from ( select id,message_data, rownumber() over(order by id) as rn from business_simulate_response_data where "+ where + " ) as a1 where a1.rn between "+startNum+" and "+endNum;
			}else if ("sqlserver2005".equalsIgnoreCase(DAOConfiguration.getDATABASE_TYPE())){
				sql = "select top " + pageLength + " * from(select row_number() over (order by id) as rownumber, id,message_data from business_simulate_response_data  where " + where +" ) A where rownumber > "  + startNum ;
			}else{
				throw new RuntimeException("the DAOCodeGenerator support mysql,oracle,mssql,db2,sqlserver2005. but batabase_type is "+ DAOConfiguration.getDATABASE_TYPE() );
			}
			...
			if(DAOConfiguration.DEBUG){
				endTime = System.currentTimeMillis();
				debug("BusinessSimulateResponseDataDAO.findByWhere()(pageNum:"+pageNum+",row count "+pageLength+") spend "+(endTime - startTime)+"ms. SQL:"+sql+"; parameter : "+where);
			}
			while (resultSet.next()) {
				returnDTO = new BusinessSimulateResponseData();
				returnDTO.setId(resultSet.getString("id"));
	...
	public long getTotalRecords(){
		...
		try {
			...
			if(DAOConfiguration.DEBUG){
				endTime = System.currentTimeMillis();
				debug("BusinessSimulateResponseDataDAO.getTotalRecords() spend "+(endTime - startTime)+"ms. SQL:"+sql+"; ");
			}
