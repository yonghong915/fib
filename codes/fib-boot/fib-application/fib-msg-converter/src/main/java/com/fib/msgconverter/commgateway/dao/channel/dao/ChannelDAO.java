...
public class ChannelDAO extends AbstractDAO {
	...
	public int insert(Channel channel) {
		...
		if( channel.getMessageCodeRecognizerId() != null && channel.getMessageCodeRecognizerId().length() < 10 ){
			throw new IllegalArgumentException("messageCodeRecognizerId too short"+channel.getMessageCodeRecognizerId());
		}
		if( channel.getMessageCodeRecognizerId() != null && channel.getMessageCodeRecognizerId().length() > 10 ){
			throw new IllegalArgumentException("messageCodeRecognizerId too long"+channel.getMessageCodeRecognizerId());
		}
		if( channel.getReturnCodeRecognizerId() != null && channel.getReturnCodeRecognizerId().length() < 10 ){
			throw new IllegalArgumentException("returnCodeRecognizerId too short"+channel.getReturnCodeRecognizerId());
		}
		if( channel.getReturnCodeRecognizerId() != null && channel.getReturnCodeRecognizerId().length() > 10 ){
		...
		try {
			...
			if(DAOConfiguration.DEBUG){
				...
				debug("ChannelDAO.insert() spend "+(endTime - startTime)+"ms. retFlag = " + retFlag + " SQL:"+sql+"; parameters : id = \""+channel.getId() +"\",name = \""+channel.getName() +"\",channel_id = \""+channel.getChannelId() +"\",startup = \""+channel.getStartup() +"\",class_name = \""+channel.getClassName() +"\",mode = \""+channel.getMode() +"\",description = \""+channel.getDescription() +"\",message_code_recognizer_id = \""+channel.getMessageCodeRecognizerId() +"\",return_code_recognizer_id = \""+channel.getReturnCodeRecognizerId() +"\",mb_mapping_group = \""+channel.getMbMappingGroup() +"\",connector_id = \""+channel.getConnectorId() +"\",channel_type = \""+channel.getChannelType() +"\",deploy_path = \""+channel.getDeployPath() +"\",event_handler_class_name = \""+channel.getEventHandlerClassName() +"\",default_processor_id = \""+channel.getDefaultProcessorId() +"\" ");
			}
	...
	public int[] insertBatch(List<Channel> channelList) {
		...
		try {
			...
			if(DAOConfiguration.DEBUG){
				endTime =System.currentTimeMillis();
				debug("ChannelDAO.insertBatch() spend "+(endTime - startTime)+"ms. retFlag = " + retFlag + " SQL:"+sql+";" );
			}
			return retFlag;
		} catch (SQLException e) {
			 e.printStackTrace(System.err);
	...
	public int update(Channel channel) {
		...
		try {
			...
			if(DAOConfiguration.DEBUG){
				endTime =System.currentTimeMillis();
				debug("ChannelDAO.update() spend "+(endTime - startTime)+"ms. retFlag = " + retFlag + " SQL:"+sql+"; parameters : id = \""+channel.getId() +"\",name = \""+channel.getName() +"\",channel_id = \""+channel.getChannelId() +"\",startup = \""+channel.getStartup() +"\",class_name = \""+channel.getClassName() +"\",mode = \""+channel.getMode() +"\",description = \""+channel.getDescription() +"\",message_code_recognizer_id = \""+channel.getMessageCodeRecognizerId() +"\",return_code_recognizer_id = \""+channel.getReturnCodeRecognizerId() +"\",mb_mapping_group = \""+channel.getMbMappingGroup() +"\",connector_id = \""+channel.getConnectorId() +"\",channel_type = \""+channel.getChannelType() +"\",deploy_path = \""+channel.getDeployPath() +"\",event_handler_class_name = \""+channel.getEventHandlerClassName() +"\",default_processor_id = \""+channel.getDefaultProcessorId() +"\" ");
			}
	...
	public int dynamicUpdate(Map primaryKey, Map updateFields) {
		...
		try {
			...
			if(DAOConfiguration.DEBUG){
				...
				sbDebug.append("ChannelDAO.dynamicUpdate() spend "+(endTime - startTime)+"ms. retFlag = " + retFlag + " SQL:"+sql.toString()+"; parameters : ");
	...
	public int deleteByPK(String id) { 
		...
		try {
			...
			if(DAOConfiguration.DEBUG){
				endTime =System.currentTimeMillis();
				debug("ChannelDAO.deleteByPK() spend "+(endTime - startTime)+"ms. retFlag = " + retFlag + " SQL:"+sql+"; parameters : id = \""+id+"\" ");
			}
	...
	public Channel selectByPK (String id)  {
		...
		try {
			...
			if(DAOConfiguration.DEBUG){
				endTime =System.currentTimeMillis();
				debug("ChannelDAO.selectByPK() spend "+(endTime - startTime)+"ms. SQL:"+sql+"; parameters : id = \""+id+"\" ");
			}
			if (resultSet.next()) {
	...
	public List findAll (int pageNum, int pageLength)  {
		...
		try {
			...
			if ("oracle".equalsIgnoreCase(DAOConfiguration.getDATABASE_TYPE())){
				sql = "select * from(select A.*,ROWNUM RN from ( select * from channel ) A where rownum <= "+endNum+" ) where RN >="+startNum+" order by id";
			}else if ("mssql".equalsIgnoreCase(DAOConfiguration.getDATABASE_TYPE())){
				sql = "select * from ( select Top "+startNum+" * from (select Top "+endNum+" * from channel					 ) t1)t2 order by id";
				...
				sql = "select top " + pageLength + " * from(select row_number() over (order by id) as rownumber, id,name,channel_id,startup,class_name,mode,description,message_code_recognizer_id,return_code_recognizer_id,mb_mapping_group,connector_id,channel_type,deploy_path,event_handler_class_name,default_processor_id from channel ) A where rownumber > "  + startNum ;
			}else{
			...
			if(DAOConfiguration.DEBUG){
				endTime = System.currentTimeMillis();
				debug("ChannelDAO.findAll()(pageNum:"+pageNum+",row count "+pageLength+") spend "+(endTime - startTime)+"ms. SQL:"+sql+"; ");
			}
			while (resultSet.next()) {
				returnDTO = new Channel();
				returnDTO.setId(resultSet.getString("id"));
	...
	public List findByWhere (String where) {
		...
		try {
			...
			if(DAOConfiguration.DEBUG){
				endTime = System.currentTimeMillis();
				debug("ChannelDAO.findByWhere()() spend "+(endTime - startTime)+"ms. SQL:"+sql+"; parameter : "+where);
			}
			while (resultSet.next()) {
				returnDTO = new Channel();
				returnDTO.setId(resultSet.getString("id"));
	...
	public List findByWhere (String where, int pageNum, int pageLength)  {
		...
		try {
			...
			if ("mysql".equalsIgnoreCase(DAOConfiguration.getDATABASE_TYPE())){
				...
				sql = "select * from(select A.*,ROWNUM RN from ( select * from channel where  "+ where + " ) A where rownum <= "+endNum+" ) where RN >="+startNum+" order by id";
			}else if ("mssql".equalsIgnoreCase(DAOConfiguration.getDATABASE_TYPE())){
				sql = "select * from ( select Top "+startNum+" * from (select Top "+endNum+" * from channel					 where " + where + " ) t1)t2 order by id";
			}else if ("db2".equalsIgnoreCase(DAOConfiguration.getDATABASE_TYPE())){
				sql = "select * from ( select id,name,channel_id,startup,class_name,mode,description,message_code_recognizer_id,return_code_recognizer_id,mb_mapping_group,connector_id,channel_type,deploy_path,event_handler_class_name,default_processor_id, rownumber() over(order by id) as rn from channel where "+ where + " ) as a1 where a1.rn between "+startNum+" and "+endNum;
			}else if ("sqlserver2005".equalsIgnoreCase(DAOConfiguration.getDATABASE_TYPE())){
				sql = "select top " + pageLength + " * from(select row_number() over (order by id) as rownumber, id,name,channel_id,startup,class_name,mode,description,message_code_recognizer_id,return_code_recognizer_id,mb_mapping_group,connector_id,channel_type,deploy_path,event_handler_class_name,default_processor_id from channel  where " + where +" ) A where rownumber > "  + startNum ;
			}else{
				throw new RuntimeException("the DAOCodeGenerator support mysql,oracle,mssql,db2,sqlserver2005. but batabase_type is "+ DAOConfiguration.getDATABASE_TYPE() );
			}
			...
			if(DAOConfiguration.DEBUG){
				endTime = System.currentTimeMillis();
				debug("ChannelDAO.findByWhere()(pageNum:"+pageNum+",row count "+pageLength+") spend "+(endTime - startTime)+"ms. SQL:"+sql+"; parameter : "+where);
			}
			while (resultSet.next()) {
				returnDTO = new Channel();
				returnDTO.setId(resultSet.getString("id"));
