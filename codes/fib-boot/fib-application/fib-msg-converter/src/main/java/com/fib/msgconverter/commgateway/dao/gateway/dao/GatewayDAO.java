...
public class GatewayDAO extends AbstractDAO {
	...
	public int insert(Gateway gateway) {
		...
		try {
			...
			if (DAOConfiguration.DEBUG) {
				...
				debug("GatewayDAO.insert() spend " + (endTime - startTime) + "ms. retFlag = " + retFlag + " SQL:" + sql
						+ "; parameters : id = \"" + gateway.getId() + "\",name = \"" + gateway.getName()
						+ "\",monitor_port = " + gateway.getMonitorPort() + ",max_handler_number = "
						+ gateway.getMaxHandlerNumber() + ",session_timeout = " + gateway.getSessionTimeout()
						+ ",logger_name = \"" + gateway.getLoggerName() + "\",log_level_into_db = \""
						+ gateway.getLogLevelIntoDb() + "\" ");
			}
	...
	public int[] insertBatch(List<Gateway> gatewayList) {
		...
		try {
			...
			if (DAOConfiguration.DEBUG) {
				endTime = System.currentTimeMillis();
				debug("GatewayDAO.insertBatch() spend " + (endTime - startTime) + "ms. retFlag = " + retFlag + " SQL:"
						+ sql + ";");
			}
			return retFlag;
		} catch (SQLException e) {
	...
	public int update(Gateway gateway) {
		...
		try {
			...
			if (DAOConfiguration.DEBUG) {
				endTime = System.currentTimeMillis();
				debug("GatewayDAO.update() spend " + (endTime - startTime) + "ms. retFlag = " + retFlag + " SQL:" + sql
						+ "; parameters : id = \"" + gateway.getId() + "\",name = \"" + gateway.getName()
						+ "\",monitor_port = " + gateway.getMonitorPort() + ",max_handler_number = "
						+ gateway.getMaxHandlerNumber() + ",session_timeout = " + gateway.getSessionTimeout()
						+ ",logger_name = \"" + gateway.getLoggerName() + "\",log_level_into_db = \""
	...
	public int dynamicUpdate(Map<String, String> primaryKey, Map<String, Object> updateFields) {
		...
		try {
			...
			if (DAOConfiguration.DEBUG) {
				...
				sbDebug.append("GatewayDAO.dynamicUpdate() spend " + (endTime - startTime) + "ms. retFlag = " + retFlag
	...
	public int deleteByPK(String id) {
		...
		try {
			...
			if (DAOConfiguration.DEBUG) {
				endTime = System.currentTimeMillis();
				debug("GatewayDAO.deleteByPK() spend " + (endTime - startTime) + "ms. retFlag = " + retFlag + " SQL:"
	...
	public Gateway selectByPK(String id) {
		...
		try {
			...
			if (DAOConfiguration.DEBUG) {
				endTime = System.currentTimeMillis();
				debug("GatewayDAO.selectByPK() spend " + (endTime - startTime) + "ms. SQL:" + sql
						+ "; parameters : id = \"" + id + "\" ");
			}
			if (resultSet.next()) {
	...
	public List<Gateway> findAll(int pageNum, int pageLength) {
		Connection conn = this.getConnection();
		if (null == conn) {
			throw new RuntimeException("Connection is NULL!");
		}
		...
		try {
			...
			if ("oracle".equalsIgnoreCase(DAOConfiguration.getDATABASE_TYPE())) {
				sql = "select * from(select A.*,ROWNUM RN from ( select * from gateway ) A where rownum <= " + endNum
				...
				sql = "select * from ( select Top " + startNum + " * from (select Top " + endNum
						+ " * from gateway					 ) t1)t2 order by id";
			} else if ("db2".equalsIgnoreCase(DAOConfiguration.getDATABASE_TYPE())) {
				sql = "select * from ( select id,name,monitor_port,max_handler_number,session_timeout,logger_name,log_level_into_db, rownumber() over(order by id) as rn from gateway ) as a1 where a1.rn between "
						+ startNum + " and " + endNum;
				...
				sql = "select top " + pageLength
						+ " * from(select row_number() over (order by id) as rownumber, id,name,monitor_port,max_handler_number,session_timeout,logger_name,log_level_into_db from gateway ) A where rownumber > "
						+ startNum;
			} else {
			...
			if (DAOConfiguration.DEBUG) {
				endTime = System.currentTimeMillis();
				debug("GatewayDAO.findAll()(pageNum:" + pageNum + ",row count " + pageLength + ") spend "
	...
	public List<Gateway> findByWhere(String where) {
		Connection conn = this.getConnection();
		if (null == conn) {
			throw new RuntimeException("Connection is NULL!");
		}
		...
		try {
			...
			if (DAOConfiguration.DEBUG) {
				endTime = System.currentTimeMillis();
				debug("GatewayDAO.findByWhere()() spend " + (endTime - startTime) + "ms. SQL:" + sql + "; parameter : "
						+ where);
			}
			while (resultSet.next()) {
				returnDTO = new Gateway();
	...
	public List<Gateway> findByWhere(String where, int pageNum, int pageLength) {
		Connection conn = this.getConnection();
		if (null == conn) {
			throw new RuntimeException("Connection is NULL!");
		}
		...
		try {
			...
			if ("mysql".equalsIgnoreCase(DAOConfiguration.getDATABASE_TYPE())) {
				...
				sql = "select * from(select A.*,ROWNUM RN from ( select * from gateway where  " + where
						+ " ) A where rownum <= " + endNum + " ) where RN >=" + startNum + " order by id";
			} else if ("mssql".equalsIgnoreCase(DAOConfiguration.getDATABASE_TYPE())) {
				sql = "select * from ( select Top " + startNum + " * from (select Top " + endNum
						+ " * from gateway					 where " + where + " ) t1)t2 order by id";
			} else if ("db2".equalsIgnoreCase(DAOConfiguration.getDATABASE_TYPE())) {
				sql = "select * from ( select id,name,monitor_port,max_handler_number,session_timeout,logger_name,log_level_into_db, rownumber() over(order by id) as rn from gateway where "
						+ where + " ) as a1 where a1.rn between " + startNum + " and " + endNum;
			} else if ("sqlserver2005".equalsIgnoreCase(DAOConfiguration.getDATABASE_TYPE())) {
				sql = "select top " + pageLength
						+ " * from(select row_number() over (order by id) as rownumber, id,name,monitor_port,max_handler_number,session_timeout,logger_name,log_level_into_db from gateway  where "
			...
			if (DAOConfiguration.DEBUG) {
				endTime = System.currentTimeMillis();
				debug("GatewayDAO.findByWhere()(pageNum:" + pageNum + ",row count " + pageLength + ") spend "
