package com.fib.msgconverter.commgateway.mapping;
...
import com.fib.msgconverter.commgateway.mapping.config.FieldMappingRule;
import com.fib.msgconverter.commgateway.mapping.config.MappingRule;
...
public class MappingEngine {
	...
	public Object map(Object sourceObject, MappingRule rule) {
		if (null == rule) {
			...
			throw new RuntimeException(MultiLanguageResourceBundle
					.getInstance().getString(
							"MappingEngine.map.mappingRule.null"));
		}
		if (null == sourceObject) {
			...
			throw new RuntimeException(MultiLanguageResourceBundle
					.getInstance().getString(
							"MappingEngine.map.sourceObject.null"));
		}
		if (!(sourceObject instanceof Map || sourceObject instanceof MessageBean)) {
			...
			throw new RuntimeException(MultiLanguageResourceBundle
					.getInstance().getString(
							"MappingEngine.map.sourceObject.wrong",
							new String[] { sourceObject.getClass().getName() }));
		}
		...
		try {
			for (i = 0; i < fieldMappingRules.size(); i++) {
				fieldMapping(sourceObject, destObject,
						(FieldMappingRule) fieldMappingRules.get(i));
			}
		} catch (Exception e) {
			e.printStackTrace();
			FieldMappingRule fRule = (FieldMappingRule) fieldMappingRules
					.get(i);
			// throw new RuntimeException("Field Mapping Error: "
			// + fRule.toString() + ":" + e.getMessage(), e);
			throw new RuntimeException(MultiLanguageResourceBundle
					.getInstance().getString(
							"MappingEngine.map.fieldMapping.error",
							new String[] { fRule.toString(), e.getMessage() }));
		}

		List manualMappingRules = rule.getScripts();
		for (i = 0; i < manualMappingRules.size(); i++) {
			this
					.executeScript(sourceObject, (String) manualMappingRules
							.get(i));
		}
		return destObject;
	}
	...
	private Object createTarget(MappingRule rule) {
		if (MappingRule.MAP == rule.getTargetType()) {
			...
			throw new RuntimeException(MultiLanguageResourceBundle
					.getInstance().getString(
							"MappingEngine.createTarget.targetType.unkown"));
		}
	}

	private void fieldMapping(Object sourceObject, Object destObject,
	...
	private void valueMapping(Object src, Object dest, FieldMappingRule aRule,
			...
			if (null == currentSource) {
				...
				throw new RuntimeException(
						MultiLanguageResourceBundle
								.getInstance()
								.getString(
										"MappingEngine.valueMapping.currentSource.null",
										new String[] { aRule
												.getSourceParameter() }));
			}
			src = currentSource;
		...
		if (-1 == firstListIndexInDest) {
			...
			if (needTranslate) {
					...
					if (null != aRule.getDefaultValue()) {
						...
						throw new RuntimeException(
								MultiLanguageResourceBundle
										.getInstance()
										.getString(
												"MappingEngine.valueMapping.value.null",
				...
				if (null != aRule.getValue()
					...
					value = executeScript("currentValue", value, aRule
							.getValue());
				}
			}
			setValue(dest, destFieldPath, value, aRule.getTargetClass(), aRule
					.isForcibleTypeConversion());
		} else {
			...
			String srcListPath = srcFieldPath.substring(0,
					firstListIndexInSource);
			String destListPath = destFieldPath.substring(0,
					firstListIndexInDest);
			...
			if (null == srcListObject) {
				...
				System.out.println(MultiLanguageResourceBundle.getInstance()
						.getString(
								"MappingEngine.valueMapping.sourceList.null",
								new String[] { srcListPath }));
				return;
			}
			...
			for (int i = 0; i < srcList.size(); i++) {
				...
				newRule.setFrom(srcFieldPath
						.substring(firstListIndexInSource + 3));
				newRule
						.setTo(destFieldPath
								.substring(firstListIndexInDest + 3));

				// if (null == aRule.getValue()
				// || 0 == aRule.getValue().trim().length()) {
				if (FieldMappingRule.VALUE_MAPPING == aRule.getType()
						|| FieldMappingRule.VALUE_TRANSLATE_MAPPING == aRule
								.getType()) {
					if (needTranslate) {
						newRule
								.setType(FieldMappingRule.VALUE_TRANSLATE_MAPPING);
	...
	private Object createListElement(Object dest, String destListPath) {
		if (dest instanceof Map) {
			...
			if (-1 == index) {
				...
				parent = ClassUtil.getBeanValueByPath(dest, destListPath
						.substring(0, index));
				attributeName = destListPath.substring(index + 1);
			}
			Object record = ClassUtil
					.invoke(parent, "create"
							+ StringUtil.toUpperCaseFirstOne(attributeName),
							null, null);
			return record;
		}
	}

	private void assignValue(Object dest, FieldMappingRule rule,
			boolean isScript) {
		...
		setValue(dest, destFieldPath, value, rule.getTargetClass(), rule
				.isForcibleTypeConversion());
	}
	...
	private Object replaceVariable(String value) {
		...
		if (null == variableValue) {
			...
			throw new RuntimeException(MultiLanguageResourceBundle
					.getInstance().getString(
							"MappingEngine.replaceVariable.variable.null",
							new String[] { value }));
		}
	...
	private void setValue(Object o, String name, Object value,
			Class targetClass, boolean force) {
		if (o instanceof Map) {
	 ...
	 * @return
	 */
	private Object dataTypeConvert(Object value, Class targetClass,
			boolean force) {
	...
	private Object convertString2Other(String value, Class targetClass,
			boolean force) {
		if (byteArray.getClass().equals(targetClass)) {
			// throw new RuntimeException("String can't convert to byte[]!");
			throw new RuntimeException(MultiLanguageResourceBundle
					.getInstance().getString(
							"MappingEngine.convertString2Other.canNotToByte[]"));
		} else if (Double.class.equals(targetClass)
				|| Double.TYPE.equals(targetClass)) {
			return new Double(value);
		} else if (Float.class.equals(targetClass)
				|| Float.TYPE.equals(targetClass)) {
			return new Float(value);
		} else if (Long.class.equals(targetClass)
				|| Long.TYPE.equals(targetClass)) {
			...
			throw new RuntimeException(MultiLanguageResourceBundle
					.getInstance().getString(
							"MappingEngine.convertString2Other.canNotTo",
							new String[] { targetClass.getName() }));
		}
	}

	private Object convertByte2Other(Byte value, Class targetClass,
			boolean force) {
		if (byteArray.getClass().equals(targetClass)) {
			// throw new RuntimeException("Byte can't convert to byte[]!");
			throw new RuntimeException(MultiLanguageResourceBundle
					.getInstance().getString(
							"MappingEngine.convertByte2Other.canNotToByte[]"));
			...
			throw new RuntimeException(MultiLanguageResourceBundle
					.getInstance().getString(
							"MappingEngine.convertByte2Other.canNotTo"));
		}
	}

	private Object convertShort2Other(Short value, Class targetClass,
			boolean force) {
		if (byteArray.getClass().equals(targetClass)) {
			// throw new RuntimeException("Short can't convert to byte[]!");
			throw new RuntimeException(MultiLanguageResourceBundle
					.getInstance().getString(
							"MappingEngine.convertShort2Other.canNotToByte[]"));
			...
			if (!force) {
				...
				throw new RuntimeException(
						MultiLanguageResourceBundle
								.getInstance()
								.getString(
										"MappingEngine.convertShort2Other.shouldNotToByte"));
			}
			...
			throw new RuntimeException(MultiLanguageResourceBundle
					.getInstance().getString(
							"MappingEngine.convertShort2Other.canNotTo",
							new String[] { targetClass.getName() }));
		}
	}

	private Object convertFloat2Other(Float value, Class targetClass,
			boolean force) {
		if (byteArray.getClass().equals(targetClass)) {
			// throw new RuntimeException("Float can't convert to byte[]!");
			throw new RuntimeException(MultiLanguageResourceBundle
					.getInstance().getString(
							"MappingEngine.convertFloat2Other.canNotToByte[]"));
			...
			if (!force) {
				...
				throw new RuntimeException(
						MultiLanguageResourceBundle
								.getInstance()
								.getString(
										"MappingEngine.convertFloat2Other.shouldNotToLong"));
			}
			...
			if (!force) {
				...
				throw new RuntimeException(
						MultiLanguageResourceBundle
								.getInstance()
								.getString(
										"MappingEngine.convertFloat2Other.shouldNotToInt"));
			}
			...
			if (!force) {
				...
				throw new RuntimeException(
						MultiLanguageResourceBundle
								.getInstance()
								.getString(
										"MappingEngine.convertFloat2Other.shouldNotToShort"));
			}
			...
			if (!force) {
				...
				throw new RuntimeException(
						MultiLanguageResourceBundle
								.getInstance()
								.getString(
										"MappingEngine.convertFloat2Other.shouldNotToByte"));
			}
			return Byte.valueOf(value.byteValue());
		} else {
			...
			throw new RuntimeException(MultiLanguageResourceBundle
					.getInstance().getString(
							"MappingEngine.convertFloat2Other.canNotTo"));
		}
	}

	private Object convertDouble2Other(Double value, Class targetClass,
			boolean force) {
		if (byteArray.getClass().equals(targetClass)) {
			// throw new RuntimeException("Double can't convert to byte[]!");
			throw new RuntimeException(MultiLanguageResourceBundle
					.getInstance().getString(
							"MappingEngine.convertDouble2Other.canNotToByte[]"));
			...
			if (!force) {
				...
				throw new RuntimeException(
						MultiLanguageResourceBundle
								.getInstance()
								.getString(
										"MappingEngine.convertDouble2Other.shouldNotToLong"));
			}
			return Long.valueOf(value.longValue());
		} else if (Float.class.equals(targetClass)
				|| Float.TYPE.equals(targetClass)) {
			if (!force) {
				...
				throw new RuntimeException(
						MultiLanguageResourceBundle
								.getInstance()
								.getString(
										"MappingEngine.convertDouble2Other.shouldNotToFloat"));
			}
			return new Float(value);
		} else if (Integer.class.equals(targetClass)
				|| Integer.TYPE.equals(targetClass)) {
			if (!force) {
				...
				throw new RuntimeException(
						MultiLanguageResourceBundle
								.getInstance()
								.getString(
										"MappingEngine.convertDouble2Other.shouldNotToInt"));
			}
			return Integer.valueOf(value.intValue());
		} else if (Short.class.equals(targetClass)
				|| Short.TYPE.equals(targetClass)) {
			if (!force) {
				...
				throw new RuntimeException(
						MultiLanguageResourceBundle
								.getInstance()
								.getString(
										"MappingEngine.convertDouble2Other.shouldNotToShort"));
			}
			return Short.valueOf(value.shortValue());
		} else if (Byte.class.equals(targetClass)
				|| Byte.TYPE.equals(targetClass)) {
			if (!force) {
				...
				throw new RuntimeException(
						MultiLanguageResourceBundle
								.getInstance()
								.getString(
										"MappingEngine.convertDouble2Other.shouldNotToByte"));
			}
			return Byte.valueOf(value.byteValue());
		} else {
			...
			throw new RuntimeException(MultiLanguageResourceBundle
					.getInstance().getString(
							"MappingEngine.convertDouble2Other.canNotTo",
							new String[] { targetClass.getName() }));
		}
	}

	private Object convertLong2Other(Long value, Class targetClass,
			boolean force) {
		if (byteArray.getClass().equals(targetClass)) {
			// throw new RuntimeException("Long can't convert to byte[]!");
			throw new RuntimeException(MultiLanguageResourceBundle
					.getInstance().getString(
							"MappingEngine.convertLong2Other.canNotToByte[]"));
			...
			if (!force) {
				...
				throw new RuntimeException(
						MultiLanguageResourceBundle
								.getInstance()
								.getString(
										"MappingEngine.convertLong2Other.shouldNotToInt"));
			}
			return Integer.valueOf(value.intValue());
		} else if (Short.class.equals(targetClass)
				|| Short.TYPE.equals(targetClass)) {
			if (!force) {
				...
				throw new RuntimeException(
						MultiLanguageResourceBundle
								.getInstance()
								.getString(
										"MappingEngine.convertLong2Other.shouldNotToShort"));
			}
			return Short.valueOf(value.shortValue());
		} else if (Byte.class.equals(targetClass)
				|| Byte.TYPE.equals(targetClass)) {
			if (!force) {
				...
				throw new RuntimeException(
						MultiLanguageResourceBundle
								.getInstance()
								.getString(
										"MappingEngine.convertLong2Other.shouldNotToByte"));
			}
			return Byte.valueOf(value.byteValue());
		} else {
			...
			throw new RuntimeException(MultiLanguageResourceBundle
					.getInstance().getString(
							"MappingEngine.convertLong2Other.canNotTo",
							new String[] { targetClass.getName() }));
		}
	}

	private Object convertInt2Other(Integer value, Class targetClass,
			boolean force) {
		if (byteArray.getClass().equals(targetClass)) {
			// throw new RuntimeException("Integer can't convert to byte[]!");
			throw new RuntimeException(MultiLanguageResourceBundle
					.getInstance().getString(
							"MappingEngine.convertInt2Other.canNotToByte[]"));
			...
			if (!force) {
				...
				throw new RuntimeException(
						MultiLanguageResourceBundle
								.getInstance()
								.getString(
										"MappingEngine.convertInt2Other.shouldNotToShort"));
			}
			return Short.valueOf(value.shortValue());
		} else if (Byte.class.equals(targetClass)
				|| Byte.TYPE.equals(targetClass)) {
			if (!force) {
				...
				throw new RuntimeException(
						MultiLanguageResourceBundle
								.getInstance()
								.getString(
										"MappingEngine.convertInt2Other.shouldNotToByte"));
			}
			return Byte.valueOf(value.byteValue());
		} else {
			...
			throw new RuntimeException(MultiLanguageResourceBundle
					.getInstance().getString(
							"MappingEngine.convertInt2Other.canNotTo",
							new String[] { targetClass.getName() }));
		}
	}
