...
public class Message {
	...
	public Message() {
		type = 1000;// = EnumConstants.MsgType.COMMON.getCode();
	...
	public boolean equalTo(Message message) {
		label0: {
			...
			if ((null == prePackEvent) ^ (null == message.getPrePackEvent()))
				return false;
			if (null != prePackEvent && null != message.getPrePackEvent()
					&& !prePackEvent.equalsIgnoreCase(message.getPrePackEvent()))
				return false;
			if ((null == postPackEvent) ^ (null == message.getPostPackEvent()))
				return false;
			if (null != postPackEvent && null != message.getPostPackEvent()
					&& !postPackEvent.equalsIgnoreCase(message.getPostPackEvent()))
				return false;
			if ((null == preParseEvent) ^ (null == message.getPreParseEvent()))
				return false;
			if (null != preParseEvent && null != message.getPreParseEvent()
					&& !preParseEvent.equalsIgnoreCase(message.getPreParseEvent()))
				return false;
			if ((null == postParseEvent) ^ (null == message.getPostParseEvent()))
				return false;
			if (null != postParseEvent && null != message.getPostParseEvent()
					&& !postParseEvent.equalsIgnoreCase(message.getPostParseEvent()))
				return false;
		...
		Iterator iterator1 = fields.values().iterator();
		Object obj = null;
