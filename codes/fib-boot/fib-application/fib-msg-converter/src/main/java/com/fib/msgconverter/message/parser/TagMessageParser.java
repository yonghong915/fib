...
public class TagMessageParser extends DefaultMessageParser {
				...
				if ("dynamic".equalsIgnoreCase(field.getReferenceType())) {
					...
					do {
						...
						if (null != field.getPreRowParseEvent())
							A(field, "row-pre-parse", field.getPreRowParseEvent(), null, arraylist, k2, l2);
						...
						if (null != field.getPostRowParseEvent())
							A(field, "row-post-parse", field.getPostRowParseEvent(), messagebean, arraylist, k2, l2);
						if (++l2 < k2) {
							i2 += 2;
							if (i2 < as.length)
								s2 = as[i2];
					...
					do {
						if (i3 >= j2)
							break;
						if (null != field.getPreRowParseEvent())
							A(field, "row-pre-parse", field.getPreRowParseEvent(), null, null, j2, i3);
						...
						if (null != field.getPostRowParseEvent())
							A(field, "row-post-parse", field.getPostRowParseEvent(), messagebean1, null, j2, i3);
					} while (true);
					boolean flag2 = false;
					l1 = 0;
				}
	...
	private int B(Field field) {
		...
		if (null != field.getRowNumField()) {
			...
			i = B(field1, ClassUtil.getBeanValueByPath(messageBean, field.getRowNumFieldName()));
		}
		return i;
	}
