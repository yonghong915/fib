...
public class XmlMessageParser extends AbstractMessageParser {
	...
	public MessageBean parse() {
		...
		try {
			B = saxreader.read(new InputSource(new StringReader(new String(messageData))));
	...
	public MessageBean A(Message message, Node node) {
		...
		try {
			...
			if (null != messagebean && !messagebean.getClass().equals(class1) || null == messagebean)
				messagebean = (MessageBean) class1.newInstance();
		} catch (Exception exception) {
		...
		if (s1.indexOf("@") != -1 || s1.indexOf("(") != -1) {
			...
			if ("dynamic".equalsIgnoreCase(field.getReferenceType())) {
				...
				for (int i1 = 0; i1 < i; i1++) {
					...
					if (null != field.getPostRowParseEvent())
						A(field, "row-post-parse", field.getPostRowParseEvent(), messagebean, messagebean3, arraylist,
								i, i1);
				}
				...
				for (int j1 = 0; j1 < i; j1++) {
					...
					if (null != field.getPostRowParseEvent())
						A(field, "row-post-parse", field.getPostRowParseEvent(), messagebean, messagebean2, null, i,
								j1);
				}
			...
			if (field1.getXpath().indexOf("${") == -1) {
				...
				for (int l1 = 0; l1 < list.size(); l1++) {
					if (null != field.getPreRowParseEvent())
						A(field, "row-pre-parse", field.getPreRowParseEvent(), messagebean, null, arraylist2,
								list.size(), l1);
					j++;
					...
					if (null != field.getPostRowParseEvent())
						A(field, "row-post-parse", field.getPostRowParseEvent(), messagebean, messagebean4, arraylist2,
								list.size(), l1);
				}
				...
				do {
					...
					arraylist1.add(messagebean1);
					if (null != field.getPostRowParseEvent())
						A(field, "row-post-parse", field.getPostRowParseEvent(), messagebean, messagebean1, arraylist1,
								k1, k1);
	...
	private void A(MessageBean messagebean, Field field, Class class1, Object obj) {
		if (null != class1 && null != obj)
			try {
				Method method = messagebean.getClass().getMethod((new StringBuilder()).append("set")
						.append(StringUtil.toUpperCaseFirstOne(field.getName())).toString(), new Class[] { class1 });
	...
	private Class<?> A(String s) {
		...
		if ("long".equals(s))
			...
			throw new RuntimeException(MultiLanguageResourceBundle.getInstance()
					.getString("XmlMessageParser.fieldType.illegal", new String[] { s }));
	}
	...
	private Object A(String s, String s1) {
		...
		if ("long".equals(s1)) {
			...
			throw new RuntimeException(MultiLanguageResourceBundle.getInstance()
					.getString("XmlMessageParser.fieldType.illegal", new String[] { s1 }));
		}
	}
		...
		try {
			...
			throw new RuntimeException((new StringBuilder())
					.append(MultiLanguageResourceBundle.getInstance()
							.getString("XmlMessageParser.getFieldClass.class.notFound", new String[] { s }))
	...
	protected void A(Field field, String s, String s1, MessageBean messagebean, MessageBean messagebean1, List list,
			int i, int j) {
		BeanShellEngine beanshellengine = new BeanShellEngine();
		beanshellengine.set("messageParser", this);
		beanshellengine.set("message", message);
