...
public class DefaultMessageParser extends AbstractMessageParser {
	...
	public MessageBean parse() {
		Assert.notNull(message, () -> new IllegalArgumentException(
				MultiLanguageResourceBundle.getInstance().getString("parameter.null", new String[] { "message" })));

		Assert.notNull(messageData, () -> new IllegalArgumentException(
				MultiLanguageResourceBundle.getInstance().getString("parameter.null", new String[] { "messageData" })));
	...
	protected int B(Field field, int i) {
		...
		if ("dynamic".equalsIgnoreCase(field.getReferenceType())) {
			...
			default:
				throw new CommonException(MultiLanguageResourceBundle.getInstance().getString("field.fieldType.notExist",
	...
	protected int A(Field field, int i) {
		...
		if (null != field.getRefLengthFieldName()) {
			...
			if (null == obj1)
				throw new CommonException((new StringBuilder()).append("field[@name='").append(field1.getName()).append("'] ")
		...
		if (field.getReference() != null && 1001 == field.getReference().getType()) {
			...
			if (j - i != ai[0])
				throw new CommonException(MultiLanguageResourceBundle.getInstance().getString(
						...
						new String[] { field.getName(), String.valueOf(j - i), (new StringBuilder()).append("").append(ai[0]).toString() }));
		}
		return j;
	}
	...
	protected int parseIntValue(Field field, Object obj) {
		...
		case 3006:
		default:
			throw new CommonException(MultiLanguageResourceBundle.getInstance().getString("parseIntValue.dataType.unsupport",
		...
		case 3011:
				...
				for (int k1 = 0; k1 < abyte8.length; k1++)
					if (abyte8[k1] != messageData[i + k1])
						throw new CommonException(MultiLanguageResourceBundle.getInstance()
								.getString("DefaultMessageParser.parseFixedFieldValue.prefix.illegal", new String[] { field.getName(),
										new String(field.getPrefix()), (new StringBuilder()).append("").append(i).toString() }));
			...
			if (!field.isStrictDataLength() && field.getPaddingDirection() == 5000) {
				...
				if (field.getSuffix() == null && field != message.getFields().get(message.getFields().size() - 1))
					throw new CommonException(MultiLanguageResourceBundle.getInstance()
							.getString("DefaultMessageParser.parseFixedFieldValue.field.mustHaveSuffix", new String[] { field.getName() }));
				if (field.getSuffix() != null) {
					...
					if (-1 == l)
						if (field == message.getFields().get(message.getFields().size() - 1))
							...
							throw new CommonException(MultiLanguageResourceBundle.getInstance()
									.getString("DefaultMessageParser.parseFixedFieldValue.field.noSuffixInMessage", new String[] { field.getName(),
											new String(field.getSuffix()), (new StringBuilder()).append("").append(i).toString() }));
					j = l - i;
				} else {
			...
			if (null != s)
				try {
					...
					throw new CommonException(
							MultiLanguageResourceBundle.getInstance().getString("field.encoding.unsupport", new String[] { field.getName(), s }));
				}
			else
