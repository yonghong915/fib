...
public class MQ4Ibm {
	...
	public static int sendMessage(String message) {
		...
		try {
			...
			// Note. The MQC interface defines all the constants used by the WebSphere MQ
			// Java programming interface

			// (except for completion code constants and error code constants).

			// MQOO_INPUT_AS_Q_DEF:Open the queue to get messages using the queue-defined
			// default.
	...
	public static String getMessage() {
		...
		try {
			...
			// //此方法读通过MQ资源管理器放入的消息会IO读写错误，通过MQ资源管理器放入的测试消息就不是UTF这样的格式的，所以会报错
