...
public class EventQueue {
	 ...
	 * 事件处理器类名
	 */
	private String eventHandlerClass = "com.fib.upp.converter.xml.event.EventHandlerThread";
...
//				handlers[i].start();
	...
	public void startDispatch(int handlerNumber) {
		...
		for (int i = 0; i < handlerNumber; i++) {
			handlers[i] = (EventHandlerThread) ReflectUtil.newInstance(eventHandlerClass);
			handlers[i].setQueue(this);
			handlers[i].setLogger(logger);
//
//			// **** add by liugl 20090226
//			// 保证每个handler线程使用和主线程相同的ClassLoader
			handlers[i].setContextClassLoader(Thread.currentThread().getContextClassLoader());
//			//
			handlers[i].start();
		}
	}
