...
//		throw exception3;
	...
	public void startElement(String s, String s1, String s2, Attributes attributes) throws SAXException {
		if ("value-range".equals(s2)) {
			...
			if (null != s3) {
				...
				if (0 == s3.trim().length())
					throw new CommonException(
							(new StringBuilder()).append(fileAbsolutePath).append(": value-range/value-ref ").append("blank").toString());
			...
			if (null == s4)
				throw new CommonException(
						(new StringBuilder()).append(fileAbsolutePath).append(": value-range/value/@value ").append("null").toString());
			s4 = s4.trim();
			if (0 == s4.length())
				throw new CommonException(
						(new StringBuilder()).append(fileAbsolutePath).append(": value-range/value/@value ").append("blank").toString());
			...
			if (s4 != null) {
				...
				if (0 == s4.length())
					throw new CommonException(
							(new StringBuilder()).append(fileAbsolutePath).append(": value-range/value/@reference ").append("blank").toString());
