package com.fib.msgconverter.util.constant;
...
public interface EnumConstant {
	...
	public enum RouteRuleType {
		...
		/** 136 STATIC 静态路由 */
		STATIC(0x88, "STATIC", "静态路由"),

		/** 137 DYNAMIC 动态路由 */
		DYNAMIC(0x89, "DYNAMIC", "客户端");

		private int code;
		private String name;
		private String text;

		RouteRuleType(int code, String name, String text) {
			this.code = code;
			this.name = name;
			this.text = text;
		}
		...
		public static RouteRuleType getRouteRuleTypeByCode(int code) {
			for (RouteRuleType mt : values()) {
				if (code == mt.getCode()) {
					return mt;
				}
			}
			throw new CommonException("Constant.RouteRule.notExist" + code);
		}

		public static RouteRuleType getRouteRuleTypeByName(String name) {
			for (RouteRuleType mt : values()) {
				if (mt.getName().equals(name)) {
					return mt;
				}
			}
			throw new CommonException("Constant.RouteRule.notExist" + name);
		}
	}
	...
	public enum DataEncTypc {
		...
		public static DataEncTypc getDataTypeByCode(int code) {
			...
			throw new CommonException("Constant.DataType.notExist" + code);
		}
		...
		public static DataEncTypc getDataTypeByName(String name) {
			...
			throw new CommonException("Constant.DataType.notExist" + name);
		}
	}
	...
	public enum ChannelType {
		...
		public static ChannelType getChannelTypeByCode(int code) {
			...
			throw new CommonException("Constant.ChannelType.notExist" + code);
		}
		...
		public static ChannelType getChannelTypeByName(String name) {
			...
			throw new CommonException("Constant.ChannelType.notExist" + name);
		}
	}
	...
	public enum ChannelMode {
		...
		public static ChannelMode getChannelModeByCode(int code) {
			...
			throw new CommonException("Constant.ChannelMode.notExist" + code);
		}
		...
		public static ChannelMode getChannelModeByName(String name) {
			...
			throw new CommonException("Constant.ChannelMode.notExist" + name);
		}
	}
	...
	public enum ConnectorMode {
		...
		/** 34 SERVER 服务端 */
		SERVER(34, "SERVER", "服务端"),

		/** 33 CLIENT 客户端 */
		CLIENT(33, "CLIENT", "客户端"),

		/** 35 DOUBLE 双向即为服务器，又为客户端 */
		DOUBLE(35, "DOUBLE", "双向");
		...
		ConnectorMode(int code, String name, String text) {
			this.code = code;
			this.name = name;
			this.text = text;
		}
		...
		public static ConnectorMode getConnectorModeByCode(int code) {
			for (ConnectorMode mt : values()) {
				if (code == mt.getCode()) {
					return mt;
				}
			}
		...
		public static ConnectorMode getConnectorModeByName(String name) {
			for (ConnectorMode mt : values()) {
				if (mt.getName().equals(name)) {
					return mt;
				}
			}
			throw new CommonException("Constant.ConnectorMode.notExist" + name);
		}
	}
}