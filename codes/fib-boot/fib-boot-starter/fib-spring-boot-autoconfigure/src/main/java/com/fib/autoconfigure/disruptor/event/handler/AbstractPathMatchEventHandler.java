...
public abstract class AbstractPathMatchEventHandler<T extends DisruptorEvent> extends AbstractAdviceEventHandler<T> implements PathProcessor<T> {
	...
	private boolean isHandlerChainContinued(T event, String path) throws Exception {
		...
		if (isEnabled(event, path)) { // isEnabled check
			...
			if (LOG.isTraceEnabled()) {
				LOG.trace("Handler '{}' is enabled for the current event under path '{}'.  "
						+ "Delegating to subclass implementation for 'onPreHandle' check.", new Object[] { getName(), path });
			}
		...
		if (LOG.isTraceEnabled()) {
			LOG.trace("Handler '{}' is disabled for the current event under path '{}'.  "
					+ "The next element in the HandlerChain will be called immediately.", new Object[] { getName(), path });
		}
