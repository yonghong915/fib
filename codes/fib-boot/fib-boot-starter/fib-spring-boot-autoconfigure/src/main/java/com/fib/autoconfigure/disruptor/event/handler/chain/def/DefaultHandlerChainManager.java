package com.fib.autoconfigure.disruptor.event.handler.chain.def;
...
import com.fib.autoconfigure.disruptor.event.NamedHandlerList;
...
public class DefaultHandlerChainManager implements HandlerChainManager<DisruptorEvent> {
	...
	/**
	 * Splits the comma-delimited handler chain definition line into individual
	 * handler definition tokens.
	 * 
	...
	public void addToChain(String chainName, String handlerName) {
		...
		if (handler == null) {
			throw new IllegalArgumentException("There is no handler with name '" + handlerName + "' to apply to chain [" + chainName
