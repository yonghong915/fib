package com.fib.autoconfigure.msgconverter.channel.message.util;

import java.util.Properties;

import com.fib.autoconfigure.msgconverter.message.metadata.Constant;
...
import com.fib.autoconfigure.msgconverter.util.MultiLanguageResourceBundle;
...
public class FieldDataLocator {
	...
	public byte[] locate(byte[] message) {
		...
		default:
			throw new CommonException("Unsupport Locate Method :" + locateMethod);
		}
	}
	...
	private byte[] locateBySeparator(byte[] message) {
		if (null == prefix) {
			throw new IllegalArgumentException("prefix is null! please set it first!");
		}
		if (null == suffix) {
			throw new IllegalArgumentException("suffix is null! please set it first!");
		}
		int startIndex = CodeUtil.byteArrayIndexOf(message, prefix, 0);
		if (-1 == startIndex) {
			throw new CommonException("can't find prefix in message! prefix="
					+ (prefixDataType == Constant.DTA_TYP_STR ? new String(prefix) : new String(CodeUtil.bytetoHex(prefix))));
		}
		startIndex += prefix.length;
		int endIndex = CodeUtil.byteArrayIndexOf(message, suffix, startIndex);
		if (-1 == endIndex) {
			throw new CommonException("can't find suffix in message! suffix="
					+ (suffixDataType == Constant.DTA_TYP_STR ? new String(suffix) : new String(CodeUtil.bytetoHex(suffix))));
		}
		...
		if (Constant.DTA_TYP_NUM == dataType && Constant.DTA_ENC_TYP_BCD == dataEncoding) {
	...
	public String locateAsString(byte[] message) {
		...
		default:
			throw new RuntimeException("Unsupport Locate Method :" + locateMethod);
		}
	}
	...
	private byte[] locateByIndex(byte[] message) {
		if (dataLength <= 0) {
			throw new IllegalArgumentException("dataLength[" + dataLength + "] <= 0");
		}
		if (index >= message.length) {
			throw new IllegalArgumentException("index[" + index + "] >= message.length[" + message.length + "] ");
		}
		...
		if (index + realLen > message.length) {
			throw new IllegalArgumentException("index+dataRealLen[" + (index + realLen) + "] >= message.length[" + message.length + "] ");
		}
	...
	/**
	 * @param locateMethod the locateMethod to set
	 */
	...
	public void setProperties(Properties properties) {
		...
		if (null == value) {
			...
			throw new IllegalArgumentException(
					MultiLanguageResourceBundle.getInstance().getString("inputParameter.null", new String[] { LOCATE_METHOD_TEXT }));
		}
		if (LOCATE_METHOD_INDEX_TEXT.equals(value)) {
			...
			throw new IllegalArgumentException(LOCATE_METHOD_TEXT + "[" + value + "] is wrong! it must be \"" + LOCATE_METHOD_INDEX_TEXT + "\" or \""
		...
		if (value != null) {
			dataType = ConstantMB.DataType.getDataTypeByName(value).getCode();
		}
		...
		if (LOCATE_METHOD_INDEX == locateMethod) {
			...
			if (null == value) {
				...
				throw new IllegalArgumentException(MultiLanguageResourceBundle.getInstance().getString("null", new String[] { INDEX_TEXT }));
			}
			...
			if (null == value) {
				...
				if (null == value) {
					...
					// + " is null!");
					throw new IllegalArgumentException(
							MultiLanguageResourceBundle.getInstance().getString("null", new String[] { DATA_LENGTH_TEXT }));
				}
			}
			...
			if (null != value) {
				...
				if (PADDING_DIRECTION_LEFT_TEXT.equals(value)) {
					...
					throw new IllegalArgumentException(MultiLanguageResourceBundle.getInstance().getString("FieldDataLocator.paddingDirection.wrong",
							new String[] { PADDING_DIRECTION_TEXT, value }));
				}
			}
			...
			if (null == value) {
				...
				// " is null!");
				throw new IllegalArgumentException(MultiLanguageResourceBundle.getInstance().getString("null", new String[] { PREFIX_TEXT }));
			}
			String separatorDataType = properties.getProperty(PREFIX_DATA_TYPE_TEXT);
			...
			if (Constant.DTA_TYP_STR_TXT.equalsIgnoreCase(separatorDataType)) {
			...
			} else if (Constant.DTA_TYP_BIN_TXT.equalsIgnoreCase(separatorDataType)) {
				...
				throw new IllegalArgumentException(MultiLanguageResourceBundle.getInstance().getString("FieldDataLocator.dataType.wrong",
						new String[] { PREFIX_DATA_TYPE_TEXT, separatorDataType }));
			}
			...
			if (null == value) {
				...
				// " is null!");
				throw new IllegalArgumentException(MultiLanguageResourceBundle.getInstance().getString("null", new String[] { SUFFIX_TEXT }));
			}
			...
			if (Constant.DTA_TYP_STR_TXT.equalsIgnoreCase(separatorDataType)) {
				...
				// + "] is wrong! it must be \"str\" or \"bin\"!");
				throw new IllegalArgumentException(MultiLanguageResourceBundle.getInstance().getString("FieldDataLocator.dataType.wrong",
						new String[] { SUFFIX_DATA_TYPE_TEXT, separatorDataType }));
			}
