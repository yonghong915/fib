...
//		}
		Document doc = null;
		try {
			doc = Dom4jUtils.getXPathDocument(xml);
		...
		String value = Dom4jUtils.getXPathValue(doc, xpathExpression);
		return value;
	}
	...
	private void parseParameters() {
		...
		if (null != tmp) {
			...
			if (0 == tmp.length()) {
				throw new RuntimeException(MultiLanguageResourceBundle
						.getInstance().getString("parameter.blank",
								new String[] { PARAM_TEXT_MSG_CHARSET }));
			}
			...
			try {
				...
				throw new RuntimeException(MultiLanguageResourceBundle
						.getInstance().getString(
								"XPathRecognizer.charset.illegal",
								new String[] { tmp, e.getMessage() }), e);
			}
