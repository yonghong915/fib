package com.fib.gateway.channel.nio;
...
import com.fib.gateway.channel.nio.config.SocketChannelConfig;
import com.fib.gateway.channel.nio.config.SocketServerChannelConfigParser;
...
public class SocketServerChannel extends Channel {
	...
	public void sendRequestMessage(byte[] requestMessage, boolean sync, int timeout) {
		throw new RuntimeException("SocketServerChannel [" + mainConfig.getId() + "] can't send request message!");
	}
	...
	public void start() {
		...
		executor.setCommBufferSize(config.getConnectionConfig().getCommBufferSize());
		// reader
					...
					try {
						...
						if (selector.isOpen()) {
							...
							logger.error(MultiLanguageResourceBundle.getInstance()
									.getString("SocketChannel.selector.select.error"), e);
							continue;
						} else {
							// logger.fatal("Selector crashed!", e);
//							logger.fatal(MultiLanguageResourceBundle.getInstance()
//									.getString("SocketChannel.selector.crashed"), e);
					...
					while (it.hasNext()) {
						...
						if (!key.isValid()) {
							...
							// + "] is invalid");
							logger.error(MultiLanguageResourceBundle.getInstance()
									.getString("SocketChannel.key.invalid", new String[] { key.toString() }));
							continue;
						}
						try {
							handleKey(key);
...
//									.getString("SocketChannel.handleKey.error"), e);
				...
				// logger.fatal(MultiLanguageResourceBundle.getInstance().getString("SocketChannel.channel.crashed"),
				// e);
		...
		public void startServer() {
			...
			try {
				...
				logger.error(MultiLanguageResourceBundle.getInstance().getString("SocketChannel.selector.open.error"),
						e);
				close();
			...
			try {
				...
				logger.error(MultiLanguageResourceBundle.getInstance()
						.getString("SocketServerChannel.handleKey.serverSocket.open.failed"), e);
				close();
				ExceptionUtil.throwActualException(e);
			}
			...
			if (key.isAcceptable()) {
				if (!key.isValid()) {
					...
					throw new RuntimeException(MultiLanguageResourceBundle.getInstance()
							.getString("SocketServerChannel.handleKey.invalid"));
				}
				...
				try {
					...
					logger.error(MultiLanguageResourceBundle.getInstance()
							.getString("SocketServerChannel.handleKey.accept.error"), e);
					if (channel != null) {
						try {
							channel.close();
				...
				try {
					...
					channel.register(selector, SelectionKey.OP_READ, readerSeed.clone());
				} catch (IOException e) {
					// e.printStackTrace();
					// logger.error("register channel error :", e);
					logger.error(MultiLanguageResourceBundle.getInstance()
							.getString("SocketServerChannel.handleKey.register.error"), e);
					try {
						channel.close();
					} catch (IOException e1) {
						// e1.printStackTrace();
				...
				try {
					...
					// e1.printStackTrace();
					logger.error(
							MultiLanguageResourceBundle.getInstance().getString("Socket.setReceiveBufferSize.error"));
				}
				...
				try {
					...
					// e1.printStackTrace();
					logger.error(MultiLanguageResourceBundle.getInstance().getString("Socket.setSendBufferSize.error"));
				}
				...
				try {
					...
					logger.error(MultiLanguageResourceBundle.getInstance().getString("SocketChannel.read.failed",
...
//									.filtSensitiveInfo(new String(reader.getMessage()),
...
//								new String[] { channel.toString(), CodeUtil.Bytes2FormattedText(msg4Log) }));
//					}
				...
				try {
					...
					logger.error(MultiLanguageResourceBundle.getInstance().getString("SocketChannel.write.failed"),
							excp);
					try {
						channel.close();
					} catch (IOException e) {
					...
					onResponseMessageSendException(channel, writer.getMessage(), excp);
					return;
				}
				if (complete) {
					// logger
...
//						if (CommGateway.isShieldSensitiveFields()) {
//							msg4Log = SensitiveInfoFilter
//									.filtSensitiveInfo(new String(writer.getMessage()),
//											CommGateway.getSensitiveFields(), CommGateway.getSensitiveReplaceChar())
//									.getBytes();
//						} else {
...
//					}
		...
		/**
		 * @param commBuffer the commBuffer to set
		 */
		...
		/**
		 * @param commBufferSize the commBufferSize to set
		 */
		...
		/**
		 * @param readerSeed the readerSeed to set
		 */
		...
		/**
		 * @param writerSeed the writerSeed to set
		 */
