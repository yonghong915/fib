...
public abstract class AbstractNioWriter implements NioWriter, Cloneable {
	 ...
	 * 
	 * @see
	 * com.giantstone.commgateway.channel.nio.writer.NioWriter#setMessage(byte
	 * [])
	 */
						...
						.debug(MultiLanguageResourceBundle
								...
								.getString(
										...
										new String[] {
												filterClassName,
												CodeUtil
														.Bytes2FormattedText(this.message) }));
			}
	 ...
	 * 
	 * @see
	 * com.giantstone.commgateway.channel.nio.writer.NioWriter#write(java.nio
	...
	public boolean write(SocketChannel channel, ByteBuffer commBuffer) {
		...
		commBuffer
				.put(
						message,
						hasWrite,
						(message.length - hasWrite) > commBuffer.capacity() ? commBuffer
								.capacity()
		...
		if (!complete) {
			if (logger.isInfoEnabled()) {
				...
				// hasWrite);
				logger
						.info(MultiLanguageResourceBundle.getInstance()
								.getString(
										"AbstractNioWriter.write.hasWrite",
										new String[] { "" + onceWrite,
												"" + hasWrite }));
			}
		}
		return complete;
