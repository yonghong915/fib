	...
	private void configCheck() {
		if (0 == config.getQuqueConfig().size()) {
			...
			throw new RuntimeException(MultiLanguageResourceBundle.getInstance().getString(
					"MQChannelConfigParser.configCheck.count.moreThan2", new String[] { fileName + ": <queue>" }));
		} else {
			...
			while (iter.hasNext()) {
				...
				if (null != tmp) {
					if (!recognizerTable.containsKey(tmp)) {
						...
						queueConfig
								.setMessageKeyRecognizer(createRecognizer((RecognizerConfig) recognizerTable.get(tmp)));
					}
			...
			if (TYPE_DOUBLE == config.getType()) {
				if (null == config.getCodeRecognizerConfig()) {
					throw new RuntimeException(MultiLanguageResourceBundle.getInstance()
							.getString("MQChannelConfigParser.configCheck.type.notDouble", new String[] { fileName }));
				}
		...
		if ("mq-channel".equalsIgnoreCase(name)) {
			...
			if (null != attr) {
				...
				if (0 == attr.length()) {
					...
					throw new RuntimeException(MultiLanguageResourceBundle.getInstance().getString("config.blank",
							new String[] { fileName, "mq-channel/@ccsid" }));
				}
				attr = getRealValue(attr);
			...
			if (null != attr) {
				...
				if (0 == attr.length()) {
					...
					throw new RuntimeException(MultiLanguageResourceBundle.getInstance().getString("config.blank",
							new String[] { fileName, "mq-channel/@timeout" }));
				}
				attr = getRealValue(attr);
			...
			if (null == attr) {
				...
				throw new RuntimeException(MultiLanguageResourceBundle.getInstance().getString("config.null",
						new String[] { fileName, "mq-channel/queue[" + queuePos + "]/@name" }));
			} else {
				attr = attr.trim();
				if (0 == attr.length()) {
					// throw new RuntimeException(fileName +
				...
				if (null != config.getQuqueConfig().get(attr)) {
					...
					throw new RuntimeException(MultiLanguageResourceBundle.getInstance().getString("reduplicate",
							new String[] { fileName + ": mq-channel/queue[" + queuePos + "]" }));
				}
			}
			...
			if (null == attr) {
				...
				throw new RuntimeException(MultiLanguageResourceBundle.getInstance().getString("config.null",
						new String[] { fileName, "mq-channel/queue[" + queuePos + "]/@type" }));
			} else {
				attr = attr.trim();
				if (0 == attr.length()) {
					// throw new RuntimeException(fileName +
			...
			if (null == attr) {
				...
				throw new RuntimeException(MultiLanguageResourceBundle.getInstance().getString("config.null",
						new String[] { fileName, "mq-channel/queue[" + queuePos + "]/@server-address" }));
			} else {
				attr = attr.trim();
				if (0 == attr.length()) {
					// throw new RuntimeException(fileName +
			...
			if (null == attr) {
				...
				throw new RuntimeException(MultiLanguageResourceBundle.getInstance().getString("config.null",
						new String[] { fileName, "mq-channel/queue[" + queuePos + "]/@port" }));
			} else {
				attr = attr.trim();
				if (0 == attr.length()) {
					// throw new RuntimeException(fileName +
			...
			if (null == attr) {
				...
				throw new RuntimeException(MultiLanguageResourceBundle.getInstance().getString("config.null",
						new String[] { fileName, "mq-channel/queue[" + queuePos + "]/@queue-manager" }));
			} else {
				attr = attr.trim();
				if (0 == attr.length()) {
					// throw new RuntimeException(fileName +
			...
			if (null == attr) {
				...
				throw new RuntimeException(MultiLanguageResourceBundle.getInstance().getString("config.null",
						new String[] { fileName, "mq-channel/queue[" + queuePos + "]/@channel" }));
			} else {
				attr = attr.trim();
				if (0 == attr.length()) {
					// throw new RuntimeException(fileName +
			...
			if (null != attr) {
				...
				if (0 != attr.length()) {
					if (attr.startsWith("0x")) {
						queueConfig.setRelationMessageId(CodeUtil.HextoByte(attr));
					} else {
			...
			if (null != attr) {
				...
				if (0 == attr.length()) {
					throw new RuntimeException(MultiLanguageResourceBundle.getInstance().getString("config.blank",
							new String[] { fileName, "mq-channel/message-key/@recognizer-id" }));
				}
				attr = getRealValue(attr);
			...
			if (null == attr) {
				throw new RuntimeException(MultiLanguageResourceBundle.getInstance().getString("config.null",
						new String[] { fileName, "reader-filter/@class" }));
			} else {
				attr = attr.trim();
				if (0 == attr.length()) {
					// throw new RuntimeException(fileName
...
//									.createClassInstance(attr));
			...
			config.setReaderFilter((AbstractMessageFilter) ClassUtil.createClassInstance(attr));
		} else if ("writer-filter".equals(name)) {
			String attr = attributes.getValue("class");
			if (null == attr) {
				throw new RuntimeException(MultiLanguageResourceBundle.getInstance().getString("config.null",
						new String[] { fileName, "writer-filter/@class" }));
			} else {
				attr = attr.trim();
				if (0 == attr.length()) {
					// throw new RuntimeException(fileName
...
//									.createClassInstance(attr));
			...
			config.setWriterFilter((AbstractMessageFilter) ClassUtil.createClassInstance(attr));
		}
	}
	...
	private int getQueueTypeByText(String text) {
		if ("send".equalsIgnoreCase(text)) {
			...
			throw new RuntimeException(MultiLanguageResourceBundle.getInstance().getString(
					"MQChannelConfigParser.queueType", new String[] { fileName + ": mq-channel/queue/@type" }));
		}
	}
	...
	private int getTypeByText(String text) {
		if ("server".equalsIgnoreCase(text)) {
			...
			throw new RuntimeException(MultiLanguageResourceBundle.getInstance()
					.getString("MQChannelConfigParser.channelType", new String[] { fileName + ": mq-channel/@type" }));
		}
	}
