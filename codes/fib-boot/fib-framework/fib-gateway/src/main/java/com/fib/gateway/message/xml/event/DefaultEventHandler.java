...
import com.fib.gateway.channel.AbstractEventAction;
...
import com.fib.gateway.mapping.MappingEngine;
...
import com.fib.gateway.message.xml.script.CustomerException;
import com.fib.gateway.messagehandler.AbstractMessageHandler;
import com.fib.gateway.messagehandler.ResultAfterHandler;
...
//				}
		...
		byte[] sourceResponseMessage = null;
		if (Processor.TYP_TRANSMIT == processor.getType() && !Session.TYP_INTERNAL.equals(session.getType())) {
...
//			SessionManager.terminalSession(session);
		...
		try {
			...
			if (!(Processor.TYP_TRANSMIT == processorType
					&& Processor.MSG_OBJ_TYP_MB == processor.getSourceChannelMessageObjectType())) {
...
//					.get(SessionConstants.EXTERNAL_SERIAL_NUM));
		...
		if (Processor.TYP_TRANSMIT == processorType) {
			...
			try {
				...
				updateSessionAfterException(session, SessionConstants.STATE_DST_REQ_MSG_PACK,
						"DefaultEventHandler.packDestRequest.error", e, event);
				handleException(event);
				return;
			}
	...
	protected Object parseMessage(int type, String messageBeanGroupId, String messageId, String mapCharset,
		...
		if (Processor.MSG_OBJ_TYP_MB == type) {
			...
			return com.fib.gateway.message.util.MapSerializer.deserialize(new String(message, mapCharset));
		}
	}
	...
	protected byte[] packMessage(String messageBeanGroupId, String messageId, Object data, String encoding) {
		if (data instanceof MessageBean) {
			...
			throw new RuntimeException("DefaultEventHandler.dataType.unkown");
		}
	}
