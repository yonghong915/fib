...
public class BEPS_121_Out extends MessageBean{
	...
	public com.fib.upp.cnaps2.messagebean.recv.req.BEPS_121_OutCcti getCctiAt(int index){
		return (com.fib.upp.cnaps2.messagebean.recv.req.BEPS_121_OutCcti)ccti.get(index);
	}
	...
	public com.fib.upp.cnaps2.messagebean.recv.req.BEPS_121_OutCcti createCcti(){
		com.fib.upp.cnaps2.messagebean.recv.req.BEPS_121_OutCcti newRecord = new com.fib.upp.cnaps2.messagebean.recv.req.BEPS_121_OutCcti();
		ccti.add(newRecord);
		return newRecord;
	}

	public void addCcti(com.fib.upp.cnaps2.messagebean.recv.req.BEPS_121_OutCcti newRecord){
		ccti.add(newRecord);
	}

	public void setCctiAt(int index, com.fib.upp.cnaps2.messagebean.recv.req.BEPS_121_OutCcti newRecord){
		ccti.set(index, newRecord);
	}

	public com.fib.upp.cnaps2.messagebean.recv.req.BEPS_121_OutCcti removeCctiAt(int index){
		return (com.fib.upp.cnaps2.messagebean.recv.req.BEPS_121_OutCcti)ccti.remove(index);
	}
	...
	public void validate(){
		...
		for( int i = 0; i < ccti.size(); i++){
			((com.fib.upp.cnaps2.messagebean.recv.req.BEPS_121_OutCcti)ccti.get(i)).validate();
		}
	...
	public String toString(boolean isWrap,boolean isTable){
		...
		com.fib.upp.cnaps2.messagebean.recv.req.BEPS_121_OutCcti cctiField = null;
		for( int i =0; i < ccti.size(); i++) {
			cctiField = (com.fib.upp.cnaps2.messagebean.recv.req.BEPS_121_OutCcti) ccti.get(i);
		...
		if ( 0 != tableBuf.length()){
			buf.append("<a n=\"ccti\" t=\"list\" c=\"java.util.ArrayList\" rc=\"com.fib.upp.cnaps2.messagebean.recv.req.BEPS_121_OutCcti\">");
			buf.append(tableBuf);
			buf.append("</a>");
		...
		if( 0 == buf.length()){
			...
			if ( isTable ){
				...
				buf.insert(0,"<b c=\"com.fib.upp.cnaps2.messagebean.recv.req.BEPS_121_Out\">");
			}
			buf.append("</b>");
			if( !isWrap ){
