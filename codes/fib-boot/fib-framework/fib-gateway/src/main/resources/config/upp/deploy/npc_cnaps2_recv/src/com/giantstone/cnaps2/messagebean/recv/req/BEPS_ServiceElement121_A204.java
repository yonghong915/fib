 ...
 * 银行本票业务
 */
public class BEPS_ServiceElement121_A204 extends MessageBean{
	...
	public void cover(MessageBean bean){
		BEPS_ServiceElement121_A204 newBean = (BEPS_ServiceElement121_A204) bean;
	...
	public void validate(){
		...
		if ( null == issueDate){
			...
			if ( issueDate.getBytes("UTF-8").length> 10 ) {
				...
				throw new RuntimeException(MultiLanguageResourceBundle.getInstance().getString("message.encoding.unsupport", new String[]{"BEPS_ServiceElement121_A204", "UTF-8"}));
			}
		...
		if ( null == draftType){
			throw new RuntimeException(
				MultiLanguageResourceBundle.getInstance().getString("Message.field.null", new String[]{"票据种类(draftType)"}));
		}
			...
			if ( draftType.getBytes("UTF-8").length> 4 ) {
				throw new RuntimeException(
				MultiLanguageResourceBundle.getInstance().getString("Message.field.maxLength", new String[]{"票据种类(draftType)", "4"}));
			}
			}catch (UnsupportedEncodingException e) {
				throw new RuntimeException(MultiLanguageResourceBundle.getInstance().getString("message.encoding.unsupport", new String[]{"BEPS_ServiceElement121_A204", "UTF-8"}));
			}
		...
		//票据种类指定值域检查
		if ( !"CT01".equals( draftType) && !"CT00".equals( draftType) ) {
			throw new RuntimeException(
				MultiLanguageResourceBundle.getInstance().getString("Message.field.value.in", new String[]{")票据种类(draftType)", "CT01/CT00"}));
		}
	...
	public String toString(boolean isWrap,boolean isTable){
		...
		if( 0 == buf.length()){
			...
			if ( isTable ){
				...
				buf.insert(0,"<b c=\"com.giantstone.cnaps2.messagebean.recv.req.BEPS_ServiceElement121_A204\">");
			}
			buf.append("</b>");
			if( !isWrap ){
